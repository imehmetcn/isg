...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Bu kod çalışmayacak çünkü redirect: true ile sayfa yönlendirilecek
      if (result?.error) {
      ...
      <div className="w-full">
        <Card className="overflow-hidden border-0 shadow-2xl rounded-2xl bg-white/90 backdrop-blur-sm">
          ...
          <CardHeader className="space-y-1 pb-6 pt-8 text-center">
            <div className="mx-auto mb-5 h-20 w-20 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 p-4 shadow-lg transform hover:scale-105 transition-transform duration-300"
              style={{ 
          ...
          <CardContent className="space-y-6 pb-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {error && (
                <div className="text-red-600 text-sm text-center bg-red-50/80 backdrop-blur-sm py-3 px-4 rounded-xl border border-red-200 shadow-sm">
                  {error}
                </div>
              )}
              ...
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-medium py-3 rounded-xl shadow-lg hover:shadow-blue-500/30 transition-all duration-300 flex items-center justify-center"
                disabled={isLoading}
              >
                {isLoading ? (
